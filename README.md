...
Средство для пропуска нужного времени. Да, именно пропуска, это не полноценный таймер, который работает на прерываниях и расчитывает с точностью до микросекунд. Даже по сравнению с другими таймера на millis(), он не вычисляет точные интервалы. Как показала практика, всё это не нужно в реальной жизни. Нужно посмотреть прошело ли нужное время, чтобы выполнить какое-то дейчтвие. Идея была честно взята у проекта [GyverMatrixWiFi](https://github.com/vvip-68/GyverMatrixWiFi), где меня поразила простата и взял его к себе в проект часов. Со временем код раздувался, страдал гигантизмом и опять усыхал, выбрасывая всё, что оказалось не нужно. В результате получился маленький и компактный метод.
